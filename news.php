...
<head>
  ...
  <!-- Basic -->
  <title>WCE Research Symposium on Computing | News</title>
      ...
      <div class="container">
        <div class="page-content">
          <div class="col-md-12" id="news">
              ...
              <div class="latest-posts-classic custom-carousel touch-carousel" data-appeared-items="4">

        <?PHP
        $sql = "select *from news where status='1' order by news_date desc LIMIT 0,5";
        $result = mysqli_query($conn, $sql);
      if(mysqli_num_rows($result)>0){

      while($row = mysqli_fetch_assoc($result))
      { 
        $date = date_parse($row['news_date']);
      ?>
                <div class="post-row item">
                  <div class="left-meta-post">
                    <div class="post-date"><span class="day"><?php echo $date['day']; ?></span><span class="month"><?php echo date('M', strtotime($date['month'])); ?></span></div>
                    <div class="post-type"><i class="fa fa-file"></i></div>
                  </div>
                  <h3 class="post-title"><a href="news_full.php?ref_id=<?php echo $row['news_id'];?>"><?php echo $row['news_heading'];?></a></h3>
                  <div class="post-content">
                    <p><?php echo $row['news_description']; ?> <a class="read-more" href="news_full.php?ref_id=<?php echo $row['news_id'];?>"><br/>Read More...</a></p>
                  </div>
                </div>
        <?php } } else { ?>
        <div class="post-row item">

                  <h3 class="post-title"><a href="#">No Latest News Available Currently</a></h3>
