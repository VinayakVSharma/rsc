...
<head>
  ...
  <!-- Basic -->
  <title>WCE ACM Student Chapter | My Posts</title>
      ...
      <div class="container">
        <div class="row blog-page">
          ...
          <div class="col-md-9 page-content">
           <h4 class="classic-title"><span>My Posts</span></h4>
            <table class="table table-bordered table-striped responsive">
            <th class="h4">POST ID</th>
            <th class="h4">POST TITLE</th>
            <th class="h4">POSTED ON</th>
            <th class="h4">AUTHOR</th>
            <th class="h4">STATUS</th>
            <th class="h4">CATEGORY</th>
            <th class="h4">VIEW COUNT</th>
            <th class="h4">IMAGE LINK</th>
            <th class="h4">ACTION</th>
               <?PHP
               $userid=$acm->User_Id();
        $sql = "select p.post_id,p.post_title,p.post_date,p.status,p.view_count,p.image_link,u.f_name,u.l_name, c.category_name from post p inner join users u inner join post_category c on p.author_id = u.user_id AND p.category_id = c.category_id where p.author_id ='$userid' ORDER BY p.post_id";
        $result = mysqli_query($conn, $sql);
            if(mysqli_num_rows($result)>0){
            while($row = mysqli_fetch_assoc($result))
               { ?>
             ...
             <td><?php if($row['status'] == "0") { echo "Not Active";} else {echo "Active"; } ;?></td>
             <td><?php echo $row['category_name'];?></td>
             <td><?php echo $row['view_count'];?></td>
             <td><?php echo $row['image_link'];?></td>
             <td><span class="pull-left"><a href="editPost.php?id=<?php echo $row['post_id'];?>" class="btn btn-sm btn-success"><i class="fa fa-edit"></i> Edit</a> 
             <a href="deletePost.php?id=<?php echo $row['post_id'];?>" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i> Delete</a>
             <span></td>
             </tr>
             <?php } }
             else
             {
              ...
              <tr>
              <td colspan="9">
                  <p class="text text-warning"><strong>There is no posts information available for you account!</strong></p>
                </td>
              </tr>
              <?php
             }
